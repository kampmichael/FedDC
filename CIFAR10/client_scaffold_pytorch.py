...
class PyTorchNNScaffold():
    ...
    def setUpdateRule(self, learningRate, schedule_ep, schedule_changerate, **kwargs):
        additional_params = ""
        for k in kwargs:
            additional_params += ", " + k  + "=" + str(kwargs.get(k))
        self._updateRule = ScaffoldOptimizer(self._core.parameters(), learningRate, schedule_changerate)
        #eval("optim." + updateRule + "(self._core.parameters(), lr=" + str(learningRate) + additional_params + ")")
    ...
    def update(self, sample, X, y, c, c_i) -> List: #sample is a list of indices, X,y are tensors of the full training set
        '''
        ...
        temp = {}
        for k, v in self._core.named_parameters():
            temp[k] = v.data.clone()
        self.old_named_parameters = temp
        self._updateRule.zero_grad()
        output = self._core(X[sample,:,:,:])
        loss = self._loss(output, y[sample])
        loss.backward()
        self._updateRule.step(c, c_i)    # Does the update
        self._schedule.step()
        return [loss.data.cpu().numpy(), output.data.cpu().numpy()]

    def setNamedParameters(self, named_params):   
        state_dict = self._core.state_dict()
        for k in named_params:
            state_dict[k] = named_params[k].data.clone()
        self._core.load_state_dict(state_dict)
        
        #for k,v in self._core.named_parameters():
        #    oldv = v.clone()
        #    v *= 1.0#amed_params[k]
        #    print(oldv, dict(self._core.named_parameters())[k])
    ...
    def getNamedParameters(self) -> PyTorchNNParameters:
        '''
    ...
    def getPreviousNamedParamters(self):
        return self.old_named_parameters
