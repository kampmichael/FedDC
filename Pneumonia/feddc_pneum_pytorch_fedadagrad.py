...
training_args.add_argument('--eta_global', type=float, default=1.0,
                    help='server leraning rate for FedAdam, FedYogi, FedAdagrad (default:1.0)') #default for fedadagrad is taken from experiments in Reddi et al., ADAPTIVE FEDERATED OPTIMIZATION, ICLR 2021                    
...
if (args.run_ablation is None):
    ...
    m_t = x_t.getCopy()
    m_t.scalarMultiply(0.0)#create a parameter vector consisting of entries with value 0
    v_t = args.tau**2
    for t in range(args.num_rounds):
        ...
        if t % args.aggregate_rounds == 0: #aggregation
            ...
            print("agg params norm: ",np.linalg.norm(aggParams.toVector()))
            delta = x_t.getCopy() #delta = 1/n sum_i=1^n x^i_t - x_(t-1) 
            print("x_t norm: ",np.linalg.norm(delta.toVector()))
            delta.scalarMultiply(-1.0)
            delta.add(aggParams)
            print("delta norm: ", np.linalg.norm(delta.toVector()))
            old_m_t = m_t.getCopy()
            old_m_t.scalarMultiply(args.beta1)
            print("m_(t-1) norm: ", np.linalg.norm(old_m_t.toVector()))
            m_t = delta.getCopy()
            m_t.scalarMultiply(1.0 - args.beta1)
            m_t.add(old_m_t)
            print("m_t norm: ", np.linalg.norm(m_t.toVector()))
            deltaSquared = np.linalg.norm(delta.flatten())**2
            print("delta squared: ", deltaSquared, " old v_t: ", v_t)
            ...
            print("x_t+1 norm: ",np.linalg.norm(x_t.toVector()))
            for i in range(args.num_clients): 
